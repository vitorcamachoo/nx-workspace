...
async function vercelDeploy(ref, commit) {
  ...
  await exec.exec('npx', ['vercel', ...args], {
...
async function vercelInspect(deploymentUrl) {
  ...
  options.listeners = {
    stdout: data => {
      ...
      myOutput += data.toString();
      core.info(data.toString());
      if(myOutput.contains('Inspect: ')) return myOutput
    },
    stderr: data => {
      myError += data.toString();
      core.info(data.toString());
  ...
  await Promise.resolve((res) => exec.exec('npx', args, {
      ...
      listeners: {
        stdout: data => {
          ...
          myOutput += data.toString();
          core.info(data.toString());
          if(myOutput.contains('Inspect: ')) res(myOutput)
        },
        stderr: data => {
          myError += data.toString();
          core.info(data.toString());
